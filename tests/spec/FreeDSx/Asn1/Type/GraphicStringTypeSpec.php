...
class GraphicStringTypeSpec extends ObjectBehavior
{
    function let()
    {
    ...
    {
        $this->shouldHaveType(GraphicStringType::class);
    }
    ...
    {
        $this->getTagNumber()->shouldBeEqualTo(AbstractType::TAG_TYPE_GRAPHIC_STRING);
    }
    ...
    function it_should_be_constructed_with_tag_information()
    {
        $this::withTag(1, AbstractType::TAG_CLASS_APPLICATION, false, 'foo')->shouldBeLike(
            (new GraphicStringType('foo'))->setTagNumber(1)->setTagClass(AbstractType::TAG_CLASS_APPLICATION)->setValue('foo')
        );
    }
}
