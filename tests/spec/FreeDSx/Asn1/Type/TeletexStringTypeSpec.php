...
class TeletexStringTypeSpec extends ObjectBehavior
{
    function let()
    {
    ...
    {
        $this->shouldHaveType(TeletexStringType::class);
    }
    ...
    {
        $this->getTagNumber()->shouldBeEqualTo(AbstractType::TAG_TYPE_TELETEX_STRING);
    }
    ...
    function it_should_be_constructed_with_tag_information()
    {
        $this::withTag(1, AbstractType::TAG_CLASS_APPLICATION, false, 'foo')->shouldBeLike(
            (new TeletexStringType('foo'))->setTagNumber(1)->setTagClass(AbstractType::TAG_CLASS_APPLICATION)
        );
    }
}
