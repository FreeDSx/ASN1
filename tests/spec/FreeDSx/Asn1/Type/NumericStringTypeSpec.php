...
class NumericStringTypeSpec extends ObjectBehavior
{
    function let()
    {
    ...
    {
        $this->shouldHaveType(NumericStringType::class);
    }
    ...
    function it_should_be_constructed_with_tag_information()
    {
        $this::withTag(1, AbstractType::TAG_CLASS_APPLICATION, false, 'foo')->shouldBeLike(
            (new NumericStringType('foo'))->setTagNumber(1)->setTagClass(AbstractType::TAG_CLASS_APPLICATION)->setValue('foo')
        );
    }
}
