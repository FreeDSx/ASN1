...
class PrintableStringTypeSpec extends ObjectBehavior
{
    function let()
    {
    ...
    {
        $this->shouldHaveType(PrintableStringType::class);
    }
    ...
    function it_should_be_constructed_with_tag_information()
    {
        $this::withTag(1, AbstractType::TAG_CLASS_APPLICATION, false, 'foo')->shouldBeLike(
            (new PrintableStringType('foo'))->setTagNumber(1)->setTagClass(AbstractType::TAG_CLASS_APPLICATION)->setValue('foo')
        );
    }
}
